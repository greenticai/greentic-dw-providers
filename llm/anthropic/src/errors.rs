...
/// Anthropic-provider-specific error wrapper.
#[derive(Debug, Error)]
pub enum AnthropicError {
    ...
    #[error("Anthropic Messages API request failed: {0}")]
    Api(String),
}
...
impl From<AnthropicError> for LlmError {
    fn from(value: AnthropicError) -> Self {
        ...
        match value {
            AnthropicError::Config(_) => LlmError::new(LlmErrorKind::InvalidRequest, message),
            AnthropicError::Api(_) => LlmError::new(LlmErrorKind::Provider, message),
        }
    }
}
...
mod tests {
    ...
    fn config_error_converts_to_invalid_request() {
        let err: greentic_dw_llm::LlmError = AnthropicError::config("bad config").into();
    ...
    fn api_error_converts_to_provider_error_with_prefix() {
        let err: greentic_dw_llm::LlmError = AnthropicError::api("upstream failed").into();
