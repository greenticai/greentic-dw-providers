...
pub trait OpenAiEmbeddingTransport: Send + Sync {
    fn create_embeddings(&self, config: &OpenAiEmbeddingConfig, payload: &Value) -> EmbeddingResult<Value>;
}
...
pub(crate) fn map_http_error(status: u16, body: &str) -> EmbeddingError {
    ...
    EmbeddingError::new(kind, format!("openai embeddings error {status}: {body}")).retryable(retryable)
}
