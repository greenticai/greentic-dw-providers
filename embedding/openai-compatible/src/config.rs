...
fn validate_base_url(base_url: &str) -> Result<(), OpenAiCompatibleEmbeddingError> {
    let parsed = Url::parse(base_url).map_err(|_| {
        OpenAiCompatibleEmbeddingError::config("base_url must be a valid absolute URL")
    })?;
...
mod tests {
    ...
    fn validate_requires_base_url() {
        let cfg =
            OpenAiCompatibleEmbeddingConfig::new("sk-test", "  ", "text-embedding-3-small", 10_000);
    ...
    fn validate_rejects_empty_key() {
        let cfg = OpenAiCompatibleEmbeddingConfig::new(
            "  ",
            "http://localhost:11434/v1",
    ...
    fn validate_rejects_zero_dim() {
        let mut cfg = OpenAiCompatibleEmbeddingConfig::with_defaults(
            "sk-test",
            "http://localhost:11434/v1",
            10_000,
        );
